...
const (
	...
	tailscaleKernelTUNRetryInterval            = 5 * time.Millisecond
)
...
func newTailscaleKernelTUN(name string, mtu uint32, closeHook func() error) (*tailscaleKernelTUN, error) {
	...
	if err != nil {
		...
		tunIf, err = singtun.New(options)
		if err != nil {
			return nil, errors.Join(gsoErr, err)
		}
		log.Warnln("[Tailscale] kernel TUN GSO unavailable on %s, using single-packet I/O: %v", name, gsoErr)
	}
	tun := &tailscaleKernelTUN{
		tun:       tunIf,
		name:      name,
...
func (t *tailscaleKernelTUN) Read(bufs [][]byte, sizes []int, offset int) (int, error) {
	...
	for {
		// sing-tun exposes LinuxTUN even when GSO batching is disabled; in that
		// mode BatchRead expects GSO buffers that wireguard-go did not provide.
		if linuxTun, ok := t.tun.(singtun.LinuxTUN); ok && linuxTun.BatchSize() > 1 {
			n, err := linuxTun.BatchRead(bufs, offset, sizes)
			if err == nil {
				return n, nil
			}
			if retryErr := t.waitNonblockingRetry(err); retryErr == nil {
				continue
			} else {
				return 0, retryErr
			}
...
func (t *tailscaleKernelTUN) Write(bufs [][]byte, offset int) (int, error) {
	// See Read: only use the Linux batch API when sing-tun reports real batching.
	if linuxTun, ok := t.tun.(singtun.LinuxTUN); ok && linuxTun.BatchSize() > 1 {
		for {
...
func (t *tailscaleKernelTUN) waitNonblockingRetry(err error) error {
	if !errors.Is(err, unix.EAGAIN) && !errors.Is(err, unix.EWOULDBLOCK) {
		return err
	}
	timer := time.NewTimer(tailscaleKernelTUNRetryInterval)
	defer timer.Stop()
	select {
	case <-t.closed:
		return os.ErrClosed
