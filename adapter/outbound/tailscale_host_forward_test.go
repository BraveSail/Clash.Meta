...
func TestTailscaleHostForwardOptionDecode(t *testing.T) {
	...
	err := structure.NewDecoder(structure.Option{
		...
		"name":      "test",
		"magic-dns": true,
		"magic-dns-direct": map[string]any{
			"enabled":       true,
	...
	direct := option.MagicDNSDirect
	if !direct.Enabled || len(direct.Interfaces) != 1 || direct.Interfaces[0] != "br-lan" {
		t.Fatalf("decoded magic-dns-direct interface: %+v", direct)
	}
	if direct.ProbeTimeout != 900 || direct.CacheTTL != 12 || direct.AnswerTTL != 4 {
		t.Fatalf("decoded magic-dns-direct: %+v", option.MagicDNSDirect)
	}
